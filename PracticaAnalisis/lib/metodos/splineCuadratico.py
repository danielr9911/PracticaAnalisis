...
def main():
    ...
    for i in range(n*2):
        ...
        for j in range(3):
            ...
            if cont == 1:
                ecuacion = ecuacion + "a" + str(int((k/3)+1)) + "* (" + str(x[var]) + ")^" + str(exp)
                ecuacion = ecuacion + " + "
            elif cont == 2:
                ecuacion = ecuacion + "b" + str(int((k / 3) + 1)) + "* (" + str(x[var]) + ")^" + str(exp)
                ecuacion = ecuacion + " + "
            elif cont == 3:
                ecuacion = ecuacion + "c" + str(int((k / 3) + 1))
                cont = 0
            cont = cont + 1
            exp = exp - 1
    ...
    for i in range(n-1):
        ...
        ecuacion = str(numeroEcuacion) + ") "
        for j in range(2):
            tabla[m + i][k + j] = (2 - j) * math.pow(x[var], exp)
            tabla[m + i][kaux + j] = -(2 - j) * math.pow(x[var], exp)
            if cont2 == 1:
                ecuacion = ecuacion + str(2-j) + " * a" + str(int((k/3)+1))+" * (" + str(x[var]) + ")^" + str(exp)
                ecuacion = ecuacion + " + "
                ecuacion = ecuacion + str(-(2 - j)) + " * a" + str(int((kaux / 3)) + 1) + " * (" + str(x[var]) + ")^" + str(exp)
                ecuacion = ecuacion + " + "
            elif cont2 == 2:
                ecuacion = ecuacion + "b" + str(int((k/3)+1))
                ecuacion = ecuacion + " + "
                ecuacion = ecuacion + "-b" + str(int((kaux/3)+1))
                cont2=0
            cont2 = cont2 + 1
            exp = exp - 1
        k = k+3
    ...
    marcasFin = np.array(['  ' for k in range(n*3)], dtype=str)
    #print(marcas)
    ...
    x = sustitucionRegresiva(matrizFinal, n*3)
    #print("X")
    #print(x)
    #print(marcasFin)
    coeficientes = {}
    ...
    for i in range(n):
        ...
        fila = fila + str(coeficientes[("a")+str(i+1)]) + " * X^2"
        fila = fila + " + "
        fila = fila + str(coeficientes[("b")+str(i+1)]) + " * X"
        fila = fila + " + "
    ...
    resp = 0.0
    for i in range(3):
        resp = resp + xOrden[(ind*3)+i]*math.pow(val, 3-i)
